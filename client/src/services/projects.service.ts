...
export const projectsService = {
  ...
  updateMemberRole: (projectId: string, userId: string, data: { role: string }) =>
    apiClient.put<ApiResponse<ProjectMember>>(`/projects/${projectId}/members/${userId}`, data).then((r) => r.data),

  removeMember: (projectId: string, userId: string) =>
    apiClient.delete<ApiResponse<void>>(`/projects/${projectId}/members/${userId}`).then((r) => r.data),
};
