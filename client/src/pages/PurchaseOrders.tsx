...
import { Plus, Search, Eye, Edit, Trash2, Truck, CheckCircle, Clock, AlertCircle, DollarSign, Download, Printer, RotateCcw, Grid3x3, List, MoreVertical, ExternalLink, User, ArrowLeftRight } from "lucide-react";
...
const purchaseOrdersData = [
  { id: 1, poNo: "PO-RYD-00001", supplier: "Tech Supplies Inc.", date: "Feb 20, 2024", items: 5, total: "$12,450", status: "delivered", paymentStatus: "paid", branch: "Riyadh HQ" },
  { id: 2, poNo: "PO-JED-00002", supplier: "Global Electronics", date: "Feb 18, 2024", items: 3, total: "$8,920", status: "in-transit", paymentStatus: "pending", branch: "Jeddah" },
  { id: 3, poNo: "PO-RYD-00003", supplier: "Premium Logistics", date: "Feb 15, 2024", items: 8, total: "$15,680", status: "processing", paymentStatus: "pending", branch: "Riyadh HQ" },
  { id: 4, poNo: "PO-DMM-00004", supplier: "Industrial Parts Co.", date: "Feb 10, 2024", items: 12, total: "$22,340", status: "delivered", paymentStatus: "paid", branch: "Dammam" },
  { id: 5, poNo: "PO-RYD-00005", supplier: "Office Supplies Ltd.", date: "Feb 8, 2024", items: 6, total: "$5,200", status: "processing", paymentStatus: "pending", branch: "Riyadh HQ" },
];
...
export default function PurchaseOrders() {
  ...
  const [editFormData, setEditFormData] = useState({ poNo: "", supplier: "", date: "", total: "", branchId: "" });
  const [createFormData, setCreateFormData] = useState({ supplier: "", date: "", total: "", branchId: "" });
  ...
  const handleExport = () => {
    const csv = [
      [t("orderNumber", language), "Supplier", t("date", language), "Items", t("total", language), t("status", language), "Payment Status", t("branch", language)],
      ...purchaseOrdersData.map(item => [item.poNo, item.supplier, item.date, item.items, item.total, item.status, item.paymentStatus, item.branch])
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-4 dark:bg-card bg-card shadow-sm border-0">
            <p className="text-sm font-medium text-muted-foreground">{t("Purchase Orders", language)}</p>
          ...
          <Card className="p-4 dark:bg-card bg-card shadow-sm border-0">
            <p className="text-sm font-medium text-muted-foreground">{t("Pending", language)}</p>
            <h3 className="text-2xl font-bold text-blue-600 mt-2">15</h3>
            <p className="text-xs text-muted-foreground mt-2">Outstanding</p>
          </Card>
          <Card className="p-4 dark:bg-card bg-card shadow-sm border-0">
            <p className="text-sm font-medium text-muted-foreground">{t("total", language)}</p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>{t("orderNumber", language)}</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>Supplier</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>{t("branch", language)}</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>{t("date", language)}</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>Items</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>{t("total", language)}</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>{t("status", language)}</th>
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>Payment</th>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align={isRTL ? "start" : "end"}>
                            <DropdownMenuItem onClick={() => {
                              setEditFormData({ poNo: item.poNo, supplier: item.supplier, date: item.date, total: item.total, branchId: "" });
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-foreground">Related Vendor</p>
                <p className="text-xs text-muted-foreground">View vendor details</p>
              </div>
              <ExternalLink size={14} className="text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
            </button>
            <button
              onClick={() => navigate("/stock-movement?reference_type=purchase_order")}
              className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-secondary/50 transition-colors text-left group"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">View stock movements from purchase orders</p>
              </div>
              <ExternalLink size={14} className="text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
            </button>
          </div>
        ...
        >
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">{t("date", language)}</label>
              <Input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">{t("total", language)}</label>
              <Input
                type="text"
        ...
        >
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">{t("orderNumber", language)}</label>
