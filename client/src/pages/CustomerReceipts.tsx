/**
 * Customer Receipts (inbound payments) list page.
...
export default function CustomerReceipts() {
  ...
  const {
    ...
    queryKey: [QUERY_KEYS.PAYMENTS_LIST, "inbound", branchId],
    queryFn: () =>
      paymentsService.list({ paymentType: PaymentTypeNew.INBOUND, limit: 100 }),
    enabled: !!branchId,
    staleTime: 15_000,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("payments.receipts.created", lang));
      invalidate();
      closeModal();
    },
    onError: () => toast.error(t("payments.receipts.loadFailed", lang)),
  });

  const postMutation = useMutation({
    mutationFn: (id: string) => paymentsService.post(id),
    onSuccess: () => {
      toast.success(t("payments.receipts.posted", lang));
      invalidate();
      setPostId(null);
      setDrawerOpen(false);
      setViewPayment(null);
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("payments.receipts.cancelled", lang));
      invalidate();
      setCancelId(null);
      setDrawerOpen(false);
      setViewPayment(null);
  ...
  const statusTag = (status: string) => {
    const map: Record<string, { color: string; label: string }> = {
      [PaymentStatusNew.DRAFT]: {
        color: "orange",
        label: t("payments.receipts.statusDraft", lang),
      },
      [PaymentStatusNew.POSTED]: {
        color: "green",
        label: t("payments.receipts.statusPosted", lang),
      },
      [PaymentStatusNew.CANCELLED]: {
        color: "red",
        label: t("payments.receipts.statusCancelled", lang),
      },
    };
  ...
  // ── Table columns ───────────────────────────────────────────────────────
  const columns = useReceiptColumns(
    t,
    lang,
    openView,
    statusTag,
  ...
  const breadcrumbs = [
    ...
    { label: t("payments.receipts.title", lang) },
  ];

  return (
    <DashboardLayout currentPage="CustomerReceipts" breadcrumbs={breadcrumbs}>
      <div className="flex flex-col gap-6">
        {/* ── 1. Stats Row ────────────────────────────────────────────── */}
        <Row gutter={[16, 16]}>
          {[
            {
              title: t("payments.receipts.total", lang),
              value: kpiTotal,
              suffix: "",
            ...
            {
              title: t("payments.receipts.statusPosted", lang),
              value: kpiPosted,
              suffix: "",
            ...
            {
              title: t("payments.receipts.statusDraft", lang),
              value: kpiDraft,
              suffix: "",
              icon: <FileTextOutlined />,
              iconColor: "#f59e0b",
            ...
            {
              title: t("payments.receipts.totalAmount", lang),
              ...
              <Row gutter={[12, 12]}>
                <Col xs={24} sm={12} lg={6}>
                  <Text type="secondary" className="text-xs mb-1 block">
                    {t("payments.receipts.status", lang)}
                  </Text>
                  <Segmented
                    value={statusFilter}
                    onChange={v => {
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {statusFilter !== "all" && (
                  <Tag closable onClose={() => setStatusFilter("all")}>
                    {t("payments.receipts.status", lang)}: {statusFilter}
                  </Tag>
                )}
          ...
          {selectedRows.length > 0 && (
            ...
            >
              <Text strong style={{ color: "var(--ant-color-primary)" }}>
                {selectedRows.length} {t("payments.receipts.selected", lang)}
              </Text>
      ...
      <ConfirmDialog
        ...
        title={t("payments.receipts.confirmPost", lang)}
        description={t("payments.receipts.confirmPostNote", lang)}
        confirmLabel={t("payments.receipts.post", lang)}
      ...
      <ConfirmDialog
        ...
        title={t("payments.receipts.confirmCancel", lang)}
        description={t("payments.receipts.confirmCancelNote", lang)}
        confirmLabel={t("payments.receipts.cancel", lang)}
      ...
      <Modal
        ...
        onCancel={closeModal}
        onOk={handleSubmit}
        okText={t("payments.receipts.new", lang)}
        confirmLoading={createMutation.isPending}
        width={isMobile ? "95vw" : 560}
        destroyOnHidden
        title={t("payments.receipts.new", lang)}
        styles={{ body: { paddingTop: 20 } }}
      >
        <Form form={form} layout="vertical">
          <Form.Item
            label={t("payments.receipts.partner", lang)}
            name="partnerId"
            rules={[{ required: true }]}
          >
            <Select
              ...
              placeholder={t("payments.receipts.selectPartner", lang)}
          ...
          <Row gutter={16}>
            <Col xs={24} sm={12}>
              <Form.Item
                label={t("payments.receipts.amount", lang)}
                name="amount"
            ...
            <Col xs={24} sm={12}>
              <Form.Item
                label={t("payments.receipts.date", lang)}
                name="paymentDate"
                rules={[{ required: true }]}
              >
          ...
          <Form.Item
            label={t("payments.receipts.treasuryAccount", lang)}
          ...
          <Form.Item label={t("payments.receipts.memo", lang)} name="memo">
      ...
      <Drawer
        ...
        title={
          viewPayment
            ? `${t("payments.receipts.details", lang)} -- ${viewPayment.paymentNumber ?? viewPayment.id.slice(0, 8)}`
            : ""
        }
      >
        {viewPayment && (
          <Space direction="vertical" size={16} style={{ width: "100%" }}>
            ...
            >
              ...
              >
                {t("payments.receipts.amount", lang)}
              </Text>
              <Text
                strong
                style={{
            ...
            <Row gutter={[16, 12]}>
              <Col span={12}>
                <Text type="secondary">
                  {t("payments.receipts.paymentNumber", lang)}
                </Text>
                <br />
              ...
              <Col span={12}>
                <Text type="secondary">
                  {t("payments.receipts.partner", lang)}
                </Text>
                <br />
                <Text strong>
                  {getName({
              ...
              {viewPayment.createdAt && (
                <Col span={12}>
                  <Text type="secondary">
                    {t("payments.receipts.createdAt", lang)}
                  </Text>
                  <br />
                  <Text>
            ...
            <Space style={{ marginTop: 8 }}>
              ...
              {viewPayment.status === PaymentStatusNew.DRAFT && (
                ...
                >
                  {t("payments.receipts.cancel", lang)}
                </Button>
              )}
            </Space>
          </Space>
...
function useReceiptColumns(
  ...
  return useMemo(
    () => [
      {
        title: t("payments.receipts.paymentNumber", lang),
      ...
      {
        title: t("payments.receipts.date", lang),
      ...
      {
        title: t("payments.receipts.partner", lang),
      ...
      {
        title: t("payments.receipts.amount", lang),
        dataIndex: "amount",
        width: 140,
        sorter: false,
        align: "end" as const,
      ...
      {
        title: t("payments.receipts.status", lang),
        dataIndex: "status",
        width: 120,
        sorter: false,
...
function buildReceiptActions(
  ...
  if (rec.status === PaymentStatusNew.DRAFT) {
    items.push({
      key: "post",
      label: t("payments.receipts.post", lang),
      icon: <CheckCircleOutlined />,
    ...
    items.push({
      key: "cancel",
      label: t("payments.receipts.cancel", lang),
      icon: <CloseCircleOutlined />,
      danger: true,
    ...
    return (
      <Card>
        <div className="text-center py-8 text-gray-400">
          {t("payments.receipts.noData", lang)}
        </div>
      </Card>
    );
  }
