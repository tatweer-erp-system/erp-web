...
import { Plus, Eye, Edit, Download, Printer, RotateCcw, Grid3x3, List, MoreVertical } from "lucide-react";
...
const salesOrdersData = [
  { id: 1, orderNo: "SO-RYD-00042", customer: "Ahmad Trading Co.", date: "Mar 5, 2024", items: 3, total: "$8,450", status: "completed", branch: "Riyadh HQ" },
  { id: 2, orderNo: "SO-JED-00043", customer: "Gulf Electronics", date: "Mar 4, 2024", items: 7, total: "$15,920", status: "processing", branch: "Jeddah" },
  { id: 3, orderNo: "SO-RYD-00044", customer: "Saudi Supply Chain", date: "Mar 3, 2024", items: 2, total: "$3,200", status: "pending", branch: "Riyadh HQ" },
  { id: 4, orderNo: "SO-DMM-00045", customer: "Eastern Logistics", date: "Mar 2, 2024", items: 5, total: "$12,340", status: "completed", branch: "Dammam" },
  { id: 5, orderNo: "SO-RYD-00046", customer: "National Services Ltd.", date: "Mar 1, 2024", items: 4, total: "$6,780", status: "cancelled", branch: "Riyadh HQ" },
];
...
export default function AllOrders() {
  ...
  const [editFormData, setEditFormData] = useState({ orderNo: "", customer: "", date: "", total: "", branchId: "" });
  const [createFormData, setCreateFormData] = useState({ customer: "", date: "", total: "", branchId: "" });

  const totalPages = Math.ceil(salesOrdersData.length / pageSize);
  const startIndex = (currentPage - 1) * pageSize;
  const paginatedData = salesOrdersData.slice(startIndex, startIndex + pageSize);

  const handlePrint = () => window.print();

  const handleExport = () => {
    const csv = [
      [t("orderNumber", language), "Customer", t("date", language), "Items", t("total", language), t("status", language), t("branch", language)],
      ...salesOrdersData.map(item => [item.orderNo, item.customer, item.date, item.items, item.total, item.status, item.branch])
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-4 dark:bg-card bg-card shadow-sm border-0">
            <p className="text-sm font-medium text-muted-foreground">{t("Sales Orders", language)}</p>
          ...
          <Card className="p-4 dark:bg-card bg-card shadow-sm border-0">
            <p className="text-sm font-medium text-muted-foreground">{t("Completed", language)}</p>
          ...
          <Card className="dark:bg-card bg-card shadow-sm border-0 overflow-hidden">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className={`px-6 py-4 text-sm font-semibold text-foreground ${isRTL ? "text-right" : "text-left"}`}>{t("Customers", language)}</th>
                ...
                <tbody>
                  {paginatedData.map((item) => (
                    <tr key={item.id} className="border-b border-border hover:bg-secondary/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-foreground font-medium">{item.orderNo}</td>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align={isRTL ? "start" : "end"}>
                            <DropdownMenuItem onClick={() => {
                              setEditFormData({ orderNo: item.orderNo, customer: item.customer, date: item.date, total: item.total, branchId: "" });
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-border">
              <div className="text-sm text-muted-foreground">
                Showing {startIndex + 1} to {Math.min(startIndex + pageSize, salesOrdersData.length)} of {salesOrdersData.length}
              </div>
              ...
              <Card key={item.id} className="p-4 dark:bg-card bg-card shadow-sm border-0">
                <div className="flex items-start justify-between">
                  <div className={isRTL ? "text-right" : ""}>
                    <h3 className="font-semibold text-foreground">{item.orderNo}</h3>
        ...
        >
          <div className="space-y-4">
            ...
            />
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">{t("Customers", language)}</label>
              <Input
                type="text"
