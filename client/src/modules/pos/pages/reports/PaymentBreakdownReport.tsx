...
> = {
  cash:       { label: "Cash",       color: "#10B981", bg: "#10B98110", icon: <DollarOutlined /> },
  card:       { label: "Card",       color: "#0066CC", bg: "#0066CC10", icon: <CreditCardOutlined /> },
  gift_card:  { label: "Gift Card",  color: "#A855F7", bg: "#A855F710", icon: <GiftOutlined /> },
  voucher:    { label: "Voucher",    color: "#F59E0B", bg: "#F59E0B10", icon: <TagOutlined /> },
};
...
export default function PaymentBreakdownReport() {
  ...
  const totalGiftCard  = data.find((r) => r.method === "gift_card")?.totalAmount ?? 0;
  ...
  const columns: ColumnsType<PaymentRow> = [
    ...
    },
    {
      ...
      render: (v: number, record) => (
        <span style={{ fontWeight: 700, color: METHOD_CONFIG[record.method].color }}>
          ${v.toFixed(2)}
        </span>
      ),
    },
        ...
        <div>
          ...
          <div
            style={{
              ...
              background: token.colorFillSecondary, marginTop: 4,
            }}
          >
            <div
              style={{
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        <Row gutter={[16, 16]} style={{ flex: 1 }}>
          {[
            { title: "Total Cash",         value: `$${totalCash.toFixed(2)}`,     color: "#10B981", suffix: "cash payments" },
            { title: "Total Card",         value: `$${totalCard.toFixed(2)}`,     color: "#0066CC", suffix: "card payments" },
            { title: "Total Gift Card",    value: `$${totalGiftCard.toFixed(2)}`, color: "#A855F7", suffix: "gift card payments" },
            { title: "Total Voucher Disc.", value: `$${totalVoucher.toFixed(2)}`, color: "#F59E0B", suffix: "voucher discounts" },
          ].map((s) => (
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} md={10}>
          <Card
            ...
            style={{ border: `1px solid ${token.colorBorderSecondary}`, borderRadius: token.borderRadiusLG, height: "100%" }}
          >
