...
const STATUS_CONFIG = {
  ...
  used:    { color: "#0066CC", bg: "#0066CC10", label: "Used",    icon: <ClockCircleOutlined /> },
...
export default function VouchersReport() {
  ...
  const columns: ColumnsType<Voucher> = [
    {
      ...
      render: (code: string) => (
        <span style={{
          ...
          background: token.colorFillAlter, padding: "2px 8px",
          borderRadius: 6, color: token.colorPrimary,
        }}>
          {code}
        </span>
      ),
    },
    {
      ...
      width: 130,
      render: (_, r) => (
        <Tag color={r.discountType === "percent" ? "blue" : "green"} style={{ borderRadius: 6, fontWeight: 600, fontSize: 11 }}>
          {r.discountType === "percent" ? `${r.discountValue}% Off` : `$${r.discountValue.toFixed(2)} Off`}
        </Tag>
      ),
    },
    {
    ...
    },
    {
      ...
      key: "orderRef",
      render: (v?: string) => v
        ? <span style={{ fontFamily: "monospace", fontSize: 12, color: token.colorPrimary }}>{v}</span>
        : <span style={{ color: token.colorTextTertiary }}>—</span>,
    },
    {
      title: "Expiry",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Row gutter={[16, 16]}>
        {[
          { title: "Total Issued",         value: totalIssued,                          color: token.colorPrimary, suffix: "vouchers" },
          { title: "Redeemed",             value: totalRedeemed,                         color: "#10B981",          suffix: "used" },
          { title: "Active",               value: totalActive,                           color: "#F59E0B",          suffix: "available" },
          { title: "Total Discount Given", value: `$${totalDiscountGiven.toFixed(2)}`,  color: "#EF4444",          suffix: "given out" },
        ].map((s) => (
      ...
      >
        <div style={{
          ...
          <Select value={typeFilter} onChange={setTypeFilter} style={{ width: 150, borderRadius: 8 }}>
